...
namespace op
{
    WrapperStructOutput::WrapperStructOutput(
        ...
        const String& udpPort_, const int writeImageMode_, bool triggerSave_, double imageCacheTime_,
        double triggerSaveTime_) :
        verbose{verbose_},
        ...
        imageCacheTime(imageCacheTime_),
        triggerSaveTime(triggerSaveTime_)
    {
        try
        {
            if (!writeBvh.empty())
