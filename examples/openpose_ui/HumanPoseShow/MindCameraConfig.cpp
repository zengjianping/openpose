...
MindCameraConfig::MindCameraConfig(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::MindCameraConfig),  m_scene(0), m_image_item(0)
{
...
MindCameraConfig::~MindCameraConfig()
{
    delete ui;
}

void MindCameraConfig::changeEvent(QEvent *e)
{
...
void MindCameraConfig::closeEvent(QCloseEvent * e)
{
    m_thread->stop();
...
void MindCameraConfig::camera_statues()
{
...
void MindCameraConfig::Image_process(QImage img)
{
    if(m_image_item)
    {
        //QGraphicsItem *mt;
        //m_scene->removeItem(mt);
...
int MindCameraConfig::SetCameraResolution(int offsetx, int offsety, int width, int height)
{
...
int MindCameraConfig::GUI_init_parameter(int hCamera,tSdkCameraCapbility * pCameraInfo)
{
...
int  MindCameraConfig::GUI_Update_Resolution(int hCamera,tSdkCameraCapbility * pCameraInfo)
{
...
int  MindCameraConfig::GUI_init_Resolution(int hCamera,tSdkCameraCapbility * pCameraInfo)
{
    int                     i=0;
...
int  MindCameraConfig::GUI_init_Trigger(int hCamera,tSdkCameraCapbility * pCameraInfo)
{
    int  pbySnapMode;
    int StrobeMode=0;
    int  uPolarity=0;
...
int  MindCameraConfig::GUI_init_exposure(int hCamera,tSdkCameraCapbility * pCameraInfo)
{
...
int  MindCameraConfig::GUI_init_WB(int hCamera,tSdkCameraCapbility * pCameraInfo)
{
...
int  MindCameraConfig::GUI_init_mmap(int hCamera,tSdkCameraCapbility * pCameraInfo)
{
    int                 gamma=0;
    int                 contrast=0;
...
int  MindCameraConfig::GUI_init_isp(int hCamera,tSdkCameraCapbility * pCameraInfo)
{
    BOOL        m_bHflip=FALSE;
    BOOL        m_bVflip=FALSE;
    int         m_Sharpness=0;
...
int  MindCameraConfig::GUI_set_speed(int hCamera,tSdkCameraCapbility * pCameraInfo)
{
    int         pbyFrameSpeed=0;
    //获得相机输出图像的帧率选择索引号。
...
int  MindCameraConfig::GUI_init_speed(int hCamera,tSdkCameraCapbility * pCameraInfo)
{
...
//分辨率操作
void MindCameraConfig::on_res_combobox_activated(int index)
{
...
//连续采集模式
void MindCameraConfig::on_radioButton_collect_clicked(bool checked)
{
    ui->radioButton_collect->setChecked(true);
    if(checked)
    {
...
//软触发模式
void MindCameraConfig::on_radioButton_software_trigger_clicked(bool checked)
{
    ui->radioButton_software_trigger->setChecked(true);
    if(checked)
    {
...
//硬触发模式
void MindCameraConfig::on_radioButton_trigger_hardware_clicked(bool checked)
{
...
//软触发一次操作
void MindCameraConfig::on_software_trigger_once_button_clicked()
{
...
//闪光灯自动模式
void MindCameraConfig::on_flashlight_auto_clicked(bool checked)
{
    ui->flashlight_auto->setChecked(true);
...
//闪光灯手动模式
void MindCameraConfig::on_flashlight_manual_clicked(bool checked)
{
...
//闪光灯 高电平有效
void MindCameraConfig::on_flashlight_h_clicked(bool checked)
{
    ui->flashlight_h->setChecked(true);
    if(checked){
        CameraSetStrobePolarity(g_hCamera,1);
...
//闪光灯 低电平有效
void MindCameraConfig::on_flashlight_l_clicked(bool checked)
{
    ui->flashlight_l->setChecked(true);
    if(checked){
        CameraSetStrobePolarity(g_hCamera,0);
...
//自动曝光调节操作
void MindCameraConfig::on_AE_horizontalSlider_valueChanged(int value)
{
...
//频闪操作
void MindCameraConfig::on_flicker_checkBox_clicked(bool checked)
{
    if(checked){
...
//频闪 50hz
void MindCameraConfig::on_radioButton_50HZ_clicked(bool checked)
{
    if(checked){
        //设置自动曝光时消频闪的频率。0: 50hz  1:60hz
...
//频闪 60hz
void MindCameraConfig::on_radioButton_60HZ_clicked(bool checked)
{
    if(checked){
        //设置自动曝光时消频闪的频率。0: 50hz  1:60hz
...
//曝光模式 自动
void MindCameraConfig::on_exposure_mode_auto_clicked(bool checked)
{
    ui->exposure_mode_auto->setChecked(true);
    if(checked){
        ui->exposure_auto->setEnabled(true);
...
//曝光模式 手动
void MindCameraConfig::on_exposure_mode_manual_clicked(bool checked)
{
...
//手动曝光 增益
void MindCameraConfig::on_horizontalSlider_gain_valueChanged(int value)
{
    /*
...
//手动曝光 曝光值
void MindCameraConfig::on_horizontalSlider_exposure_time_valueChanged(int value)
{
...
void MindCameraConfig::on_lineEdit_exposure_time_returnPressed()
{
...
//参数操作A按钮
void MindCameraConfig::on_radioButton_A_clicked(bool checked)
{
    ui->radioButton_A->setChecked(true);
...
//参数操作B按钮
void MindCameraConfig::on_radioButton_B_clicked(bool checked)
{
...
//参数操作C按钮
void MindCameraConfig::on_radioButton_C_clicked(bool checked)
{
...
//参数操作D按钮
void MindCameraConfig::on_radioButton_D_clicked(bool checked)
{
...
//默认参数
void MindCameraConfig::on_pushButton_para_acquiesce_released()
{
    //加载指定组的参数到相机中。
...
//保存参数
void MindCameraConfig::on_pushButton_para_save_released()
{
    /*
    存当前相机参数到指定的参数组中。相机提供了A,B,C,D
    A,B,C,D四组空间来进行参数的保存。
...
//加载参数文件
void MindCameraConfig::on_pushButton_para_load_released()
{
...
//白平衡按钮操作
void MindCameraConfig::on_AWB_once_button_released()
{   
    int RPos,GPos,BPos;
...
//白平衡 增益R
void MindCameraConfig::on_horizontalSlider_gain_r_valueChanged(int value)
{
    int RPos,GPos,BPos;
...
//白平衡 增益G
void MindCameraConfig::on_horizontalSlider_gain_g_valueChanged(int value)
{
    int RPos,GPos,BPos;
...
//白平衡 增益B
void MindCameraConfig::on_horizontalSlider_gain_b_valueChanged(int value)
{
    int RPos,GPos,BPos;
...
//白平衡 饱和度
void MindCameraConfig::on_horizontalSlider_saturation_valueChanged(int value)
{
    CameraSetSaturation(g_hCamera,value);
}

//GAMMA
void MindCameraConfig::on_gamma_horizontalSlider_valueChanged(int value)
{
    CameraSetGamma(g_hCamera,value);
}

//对比度
void MindCameraConfig::on_contrast_horizontalSlider_valueChanged(int value)
{
    /*
...
//水平镜像
void MindCameraConfig::on_checkBox_isp_h_clicked(bool checked)
{
    if(checked){
...
//垂直镜像
void MindCameraConfig::on_checkBox_isp_v_clicked(bool checked)
{
    if(checked){
...
//锐化
void MindCameraConfig::on_horizontalSlider_isp_acutance_valueChanged(int value)
{
...
//保存图片路径设置
void MindCameraConfig::on_pushButton_snap_path_released()
{
...
//保存图片按钮确认
void MindCameraConfig::on_pushButton_snap_catch_released()
{
...
void MindCameraConfig::on_radioButton_jpg_clicked()
{
    g_SaveImage_type=1;
}

void MindCameraConfig::on_radioButton_png_clicked()
{
    g_SaveImage_type=2;
}

//保存图片格式 bmp
void MindCameraConfig::on_radioButton_bmp_clicked()
{
    g_SaveImage_type=3;
}

//保存图片格式 raw
void MindCameraConfig::on_radioButton_raw_clicked()
{
    g_SaveImage_type=4;
}

//速度选择
void MindCameraConfig::radioChange()
{
