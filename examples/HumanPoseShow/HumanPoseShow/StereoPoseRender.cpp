...
{
    printf("############### initializeVisualization\n");
    try
    {
        char *my_argv[] = { NULL };
        int my_argc = 0;
        ...
        gWindowHandle = glutCreateWindow(std::string{OPEN_POSE_NAME_AND_VERSION + " - 3-D Display"}.c_str());
        initGraphics();
...
StereoPoseRender::StereoPoseRender(const bool copyGlToCvMat, const PoseModel poseModel) :
    mCopyGlToCvMat{copyGlToCvMat}
{
    try
    {
        ...
        //    sConstructorSet = true;
        //else
        //    error("The StereoPoseRender class can only be initialized once.", __LINE__, __FUNCTION__, __FILE__);
    }
    catch (const std::exception& e)
    {
        error(e.what(), __LINE__, __FUNCTION__, __FILE__);
...
StereoPoseRender::~StereoPoseRender()
{
    try
    {
        terminateVisualization();
    }
    catch (const std::exception& e)
    {
...
void StereoPoseRender::initialize()
{
    try
    {
...
void StereoPoseRender::setKeypoints(const Array<float>& poseKeypoints3D, const Array<float>& faceKeypoints3D,
...
Matrix StereoPoseRender::readCvMat()
{
    try
    {
        // 3-D rendering
