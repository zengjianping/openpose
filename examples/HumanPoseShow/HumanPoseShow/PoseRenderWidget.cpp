...
const double RAD_TO_DEG = 0.0174532925199433;

PoseRenderWidget::PoseRenderWidget(QWidget* parent)
    : QOpenGLWidget(parent)
{
}

PoseRenderWidget::~PoseRenderWidget()
{
}

void PoseRenderWidget::initializeGL()
{
    initGraphics();
}

void PoseRenderWidget::resizeGL(int w, int h)
{
    gAspectRatio = w * 1.0 / h;
}

void PoseRenderWidget::paintGL()
{
    renderMain();
}

void PoseRenderWidget::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if (gBButton1Down)
    {
        ...
        {
            gMouseXRotateDeg += (event->x() - gXClick)*0.2f;
            gMouseYRotateDeg -= (event->y() - gYClick)*0.2f;
        }
        else if (gCameraMode == CameraMode::CAM_PAN)
        {
            gMouseXPan += (event->x() - gXClick) / 2 * gScaleForMouseMotion;
            gMouseYPan -= (event->y() - gYClick) / 2 * gScaleForMouseMotion;
        }
        else if (gCameraMode == CameraMode::CAM_PAN_Z)
        {
            auto dist = std::sqrt(pow((event->x() - gXClick), 2.0f) + pow((event->y() - gYClick), 2.0f));
            if (event->y() < gYClick)
                dist *= -1;
        ...
        {
            op::opLog("gMouseXRotateDeg = " + std::to_string(gMouseXRotateDeg));
            op::opLog("gMouseYRotateDeg = " + std::to_string(gMouseYRotateDeg));
            op::opLog("gMouseXPan = " + std::to_string(gMouseXPan));
            op::opLog("gMouseYPan = " + std::to_string(gMouseYPan));
            op::opLog("gMouseZPan = " + std::to_string(gMouseZPan));
        }
    }
}

void PoseRenderWidget::mousePressEvent(QMouseEvent *event)
{
    if (event->button() == Qt::LeftButton)
    {
        ...
        if (LOG_VERBOSE_3D_RENDERER)
            op::opLog("Clicked: [" + std::to_string(gXClick) + "," + std::to_string(gYClick) + "]");
    }
}

void PoseRenderWidget::mouseReleaseEvent(QMouseEvent *event)
{
    if (event->button() == Qt::LeftButton)
    {
        gBButton1Down = false;
...
void PoseRenderWidget::wheelEvent(QWheelEvent *event)
{
    if (event->angleDelta().y() < 0)  //zoom in
        ...
        gGViewDistance -= 10 * gScaleForMouseMotion;
    if (LOG_VERBOSE_3D_RENDERER)
        op::opLog("gGViewDistance: " + std::to_string(gGViewDistance));
}

void PoseRenderWidget::setKeypoints(const op::Array<float>& poseKeypoints3D, const op::Array<float>& faceKeypoints3D,
    const op::Array<float>& leftHandKeypoints3D, const op::Array<float>& rightHandKeypoints3D)
{
    try
    {
        ...
        // From m to mm
        op::scaleKeypoints(gKeypoints3D.poseKeypoints, 1e3f);
        op::scaleKeypoints(gKeypoints3D.faceKeypoints, 1e3f);
        op::scaleKeypoints(gKeypoints3D.leftHandKeypoints, 1e3f);
        op::scaleKeypoints(gKeypoints3D.rightHandKeypoints, 1e3f);
        lock.unlock();
    }
...
void PoseRenderWidget::clearKeypoints()
{
    try
    {
        std::unique_lock<std::mutex> lock{gKeypoints3D.mutex};
        gKeypoints3D.poseKeypoints.reset();
        gKeypoints3D.faceKeypoints.reset();
        gKeypoints3D.leftHandKeypoints.reset();
        gKeypoints3D.rightHandKeypoints.reset();
        gKeypoints3D.validKeypoints = true;
        lock.unlock();
    }
    catch(const std::exception& e)
...
void PoseRenderWidget::initGraphics()
{
    if (!sConstructorSet)
    {
...
void PoseRenderWidget::drawConeByTwoPts(const cv::Point3f& pt1, const cv::Point3f& pt2, const float ptSize)
{
...
void PoseRenderWidget::renderHumanBody(const op::Array<float>& keypoints, const std::vector<unsigned int>& pairs,
        const std::vector<float> colors, const float ratio)
{
    ...
    const auto xScale = 24.f * (float)gAspectRatio;
...
void PoseRenderWidget::renderFloor()
{
...
void PoseRenderWidget::renderMain(void)
{
    ...
    if (gKeypoints3D.validKeypoints)
    {
        ...
        renderHumanBody(gKeypoints3D.faceKeypoints, op::FACE_PAIRS_RENDER, op::FACE_COLORS_RENDER, 0.5f);
        renderHumanBody(gKeypoints3D.leftHandKeypoints, op::HAND_PAIRS_RENDER, op::HAND_COLORS_RENDER, 0.5f);
        renderHumanBody(gKeypoints3D.rightHandKeypoints, op::HAND_PAIRS_RENDER, op::HAND_COLORS_RENDER, 0.5f);
    }
    lock.unlock();
...
void PoseRenderWidget::mouseButton(const int button, const int state, const int x, const int y)
{
...
void PoseRenderWidget::mouseMotion(const int x, const int y)
{
...
void PoseRenderWidget::keyPressed(const unsigned char key, const int x, const int y)
{
    try
    {
        UNUSED(x);
...
/*
void PoseRenderWidget::initializeVisualization()
{
    try
    {
        char *my_argv[] = { NULL };
...
void PoseRenderWidget::terminateVisualization()
{
    try
    {
        if (gWindowHandle >= 0)
        {
            glutDestroyWindow(gWindowHandle);
            gWindowHandle = -1;
        }
        glutLeaveMainLoop();
    }
